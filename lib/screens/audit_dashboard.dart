...
class AuditDashboardState extends State<AuditDashboard> {
  ...
  PieChartSectionData _buildPieSection(String title, dynamic value, Color color) {
  ...
  Future<void> _exportSummaryPdf() async {
    ...
    pdf.addPage(pw.MultiPage(
      build: (context) => [
        pw.Text('Alora Site Audit Summary', style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold)),
        pw.SizedBox(height: 12),
        pw.Text('Website: ${_controller.text}'),
        pw.SizedBox(height: 12),
        pw.Text('SEO Score: ${_results?['seo']['score'] ?? 'N/A'}'),
        pw.Text('Broken Links: ${_results?['links']['broken']?.length ?? 0}'),
        pw.Text('Security Findings: ${_results?['security']['nuclei']?.length ?? 0} items'),
        pw.SizedBox(height: 12),
        pw.Text('Business Profile:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
    ...
    await Printing.sharePdf(bytes: await pdf.save(), filename: 'audit_summary.pdf');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final charts = _results == null
        ? SizedBox()
        : Screenshot(
            controller: _screenshotController,
            child: AuditCharts(seo: _results!['seo'], links: _results!['links']),
          );

    final severity = _results == null
        ? SizedBox()
        : SeverityPieChart(nucleiFindings: _results!['security']['nuclei'] ?? []);
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            children: [
              TextField(controller: _controller, decoration: InputDecoration(labelText: 'Enter Website URL')),
              SizedBox(height: 12),
              ElevatedButton(onPressed: loadData, child: Text('Start Audit')),
              SizedBox(height: 12),
              // Bar Chart Section
              if (auditData != null) ...[
                  ...
                  child: BarChart(
                    BarChartData(
                      barGroups: [
                        ...
                        _buildBarGroup(2, 'Accessibility', metrics['efficiency']),
                        ...
                        bottomTitles: AxisTitles(
                          sideTitles: SideTitles(
                            ...
                            getTitlesWidget: (value, _) {
                              const labels = ['SEO', 'Broken', 'Accessibility', 'Security', 'Pages' ];
                              return Padding(
              ...
              if (_results != null) ...[
                ...
                _buildResultCard('Business Profile', _results!['business_profile']),
                _buildResultCard('Pages', _results!['pages']),
                _buildResultCard('Alt Text Suggestions', _results!['alt_text_suggestions']),
